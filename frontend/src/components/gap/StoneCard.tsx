  ...
  return (
    <Card size="small" style={{ borderColor: "var(--ochre)", marginBottom: 12 }}>
      <Space direction="vertical" size={12} style={{ width: "100%" }}>
        ...
        <Space wrap size={16}>
          <Statistic title="当前持有" value={have} valueStyle={{ fontSize: 16, color: lack === 0 ? "var(--bamboo)" : undefined }} />
          <Statistic title="升满需要" value={need} valueStyle={{ fontSize: 16 }} />
          <Statistic title="缺口" value={lack} valueStyle={{ fontSize: 16, color: lack === 0 ? "var(--bamboo)" : "var(--terracotta)" }} />
          <Statistic title="还需体力" value={lack === 0 ? "0" : fmt(lackStamina)} valueStyle={{ fontSize: 16, color: lack === 0 ? "var(--bamboo)" : "var(--terracotta)" }} />
          <Statistic title="自然恢复天数" value={lack === 0 ? "0" : `${lackDays} 天`} valueStyle={{ fontSize: 16, color: lack === 0 ? "var(--bamboo)" : "var(--terracotta)" }} />
        </Space>

        <Progress percent={pct} showInfo={false} strokeColor={lack === 0 ? "var(--bamboo)" : "var(--accent)"} />
